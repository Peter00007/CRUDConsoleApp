...
public class JavaIODeveloperRepositoryImpl implements DeveloperRepository {
    ...
    public JavaIODeveloperRepositoryImpl() {
        javaIOSkillRepositoryImpl = new JavaIOSkillRepositoryImpl();
        javaIOAccountRepositoryImpl = new JavaIOAccountRepositoryImpl();
    }
    ...
    public List<Developer> getAll() {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(FILE_PATH))) {
            ...
            while ((developerFileContent = bufferedReader.readLine()) != null) {
                ...
                if (index == 1) {
                    set.add(javaIOSkillRepositoryImpl.getById(Long.parseLong(developerRecords[4].substring(1, developerRecords[4].length() - 1))));
                } else {
                    set.add(javaIOSkillRepositoryImpl.getById(Long.parseLong(developerRecords[4].substring(1, developerRecords[4].length()))));
                }
                for (int i = 5; i < developerRecords.length - 1; i++) {
                    if (i == developerRecords.length - 2) {
                        set.add(javaIOSkillRepositoryImpl.getById(Long.parseLong(developerRecords[i].substring(0, developerRecords[i].length() - 1))));
                    } else {
                        set.add(javaIOSkillRepositoryImpl.getById(Long.parseLong(developerRecords[i])));
                    }
                }
                ...
                list.add(new Developer(Long.parseLong(developerRecords[0]), developerRecords[1], developerRecords[2],
                        developerRecords[3], set, javaIOAccountRepositoryImpl.getById(accountId)));
            }
    ...
    public void deleteByObject(Developer developer) {
        String saveDeveloperLine = null;
        StringBuffer stringBuffer = new StringBuffer();
        try (BufferedReader reader = new BufferedReader(new FileReader(new File(FILE_PATH)))) {
